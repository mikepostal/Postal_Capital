...
public class StoreDaoImpl implements StoreDao {
	...
	@Override
	public List<Store> findAllByEmailAndRole(String email, String role) {
		Session session = sessionFactory.getCurrentSession();
		String sql = "";
		if(role.equalsIgnoreCase("REGIONAL")) {
			sql = "From Store s where s.rmEmail=:email";
		}else if(role.equalsIgnoreCase("DISTRICT")){
			sql = "From Store s where s.dmEmail=:email";
		}else {
			sql = "From Store s where s.storeEmail=:email";
		}
		Query<Store> query = session.createQuery(sql, Store.class);
